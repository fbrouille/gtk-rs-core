...
use gio::{
    Cancellable, DriveStartFlags, File, FileCopyFlags, FileCreateFlags, FileMeasureFlags,
    FileMonitor, FileMonitorEvent, FileMonitorFlags, FileQueryInfoFlags, IOErrorEnum,
    MountMountFlags, MountOperation, MountUnmountFlags, prelude::*, subclass::prelude::*,
};
    ...
    let _ = glib::MainContext::new().with_thread_default(|| {
        ...
        fn eject_mountable<T: IsA<File>, P: FnOnce(Result<(), glib::Error>) + 'static>(
            ...
            >(
                ...
                unsafe {
                    let mut error = std::ptr::null_mut();
                    gio::ffi::g_file_eject_mountable_finish(
                        _source_object as *mut _,
                        res,
                        &mut error,
                    );
...
fn file_measure_disk_usage() {
    ...
    let res = my_local_dir.measure_disk_usage(
    ...
    let (mut completed, mut size, mut nb_dirs, mut nb_files) = (false, 0u64, 0u64, 0u64);
    let res = File::for_path(&my_temp_dir.path).measure_disk_usage(
        FileMeasureFlags::all(),
        Cancellable::NONE,
        Some(
            &mut move |reporting: bool, current_size, num_dirs, num_files| {
