...
use gio::{Cancellable, File, prelude::*};
#[cfg(unix)]
use gio_unix::OutputStream;
#[cfg(windows)]
use gio_win32::OutputStream;
...
impl Temp {
    ...
    pub fn create_file(tmpl: &str) -> (Self, AutoCloseableOutpuStream) {
        unsafe {
            ...
            #[cfg(unix)]
            let output_stream = OutputStream::take_fd(OwnedFd::from_raw_fd(fd)).upcast();
            #[cfg(windows)]
            let output_stream = OutputStream::take_handle(OwnedHandle::from_raw_handle(
